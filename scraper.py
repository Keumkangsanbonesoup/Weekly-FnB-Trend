...
five_days_ago_date = (now_kst - timedelta(days=7)).strftime("%Y%m%d")
...
def get_community_trends(query, max_results=7):
    ...
    try:
        ...
        res = service.cse().list(
            q=query, 
            cx=GOOGLE_CX, 
            dateRestrict="d7", # 💡 블로그/유튜브와 통일성 있게 최근 7일(d7)로 수정
            num=max_results
        ).execute()
        return [{
...
아래는 최근 7일간(1주일) 수집된 유튜브, 네이버 블로그, 커뮤니티 데이터입니다.
...
4. 확실하고 구체적인 상품명/메뉴명이 있는 것만 뽑으세요. 교차검증을 통과한 것만 출력하세요. 억지로 채우지 말고 최소 5개, 최대 8개 범위에서 품질 기준으로 선정하세요.
