...
seven_days_ago = time_limit.strftime('%Y-%m-%dT%H:%M:%SZ')
...
def get_latest_youtube_trends(keywords, max_results=20):
    ...
    request = youtube.search().list(
        ...
        order="viewCount", publishedAfter=seven_days_ago, maxResults=max_results
    )
...
아래는 최근 1주일간 유튜브에서 조회수가 높은 F&B 관련 영상 목록입니다.
...
{{"summary": "한 문장으로 이번 주 F&B 트렌드 핵심 요약", "trends": [{{"id": 1, "title": "구체적인 상품명 또는 메뉴명", "description": "이 메뉴/상품이 왜 지금 화제인지 2~3문장으로 설명", "sentiment": "hot", "keywords": ["키워드1", "키워드2", "키워드3"], "source_video": "https://youtube.com/watch?v=..."}}]}}
"""
...
def enrich_with_naver_trends(trend_data):
    """추출된 트렌드 키워드를 네이버 데이터랩으로 교차 검증합니다."""
...
def load_existing_data(filepath="data.js"):
    ...
    try:
        ...
        # "const trendData = {...};" 에서 JSON 부분만 추출
...
if __name__ == "__main__":
    try:
        ...
        print(f"   → AI 분석 완료. 트렌드 {len(new_week_data.get('trends', []))}개 추출.")
        ...
        print(f"✅ 모든 작업 완료! data.js 업데이트 성공. (총 {len(weeks)}주차 누적)")
