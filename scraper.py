...
def get_latest_youtube_trends(keywords, max_results=7):
...
4. 확실한 아이템이 2~3개뿐이라면 무리하게 5개를 채우지 말 것. (최대 5개)

[출력 JSON 형식] 반드시 JSON만 출력, 마크다운 금지:
{{"summary": "이번 주 트렌드 핵심 요약 한 문장", "trends": [{{"title": "상품명", "description": "화제 이유 2~3문장", "sentiment": "hot", "keywords": ["키워드1", "키워드2"], "mentioned_in": ["youtube"], "source_link": "URL", "source_name": "출처명"}}]}}
"""
    ...
    for model in MODEL_FALLBACKS:
        ...
        for attempt in range(max_retries):
            try:
                req = urllib.request.Request(url, data=json.dumps(data_payload).encode('utf-8'),
...
if __name__ == "__main__":
    try:
        ...
        recent_videos = get_latest_youtube_trends("편의점 신상 리뷰|CU 신상|GS25 신상|디저트 신메뉴", max_results=7)
